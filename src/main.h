...
static const int64 DUST_SOFT_LIMIT = 10000; // 0.010 PRC
...
extern int   miMandelbrot[720][365];
extern int   miMandelbrotSingle[720*365];
extern int   miHeight;        // Days per year
extern int   miWidth;         // Blocks per day
extern float miZoom;          // Fractal Zoom Level
extern int   miMaxIterations; // Fractal Detail

extern std::map<std::string, std::string> mvApplicationCache;
extern std::map<std::string, std::string> mvCashBackAddressesPaid;
...
public:
    ...
    (
        ...
        READWRITE(vout);
        READWRITE(nLockTime);
		READWRITE(SendersAddress);
		READWRITE(PeoplesReserve);
